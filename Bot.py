...
def load_message_log():
    """Đọc thông tin tin nhắn từ file setting.json."""
    try:
...
def save_message_log(message_log):
    """Lưu thông tin tin nhắn vào file setting.json."""
    try:
        with open(SETTING_FILE, 'r', encoding='utf-8') as f:
            settings = json.load(f)
    except FileNotFoundError:
...
def bot_on_group(bot, thread_id):
    ...
    try:
        ...
        if thread_id not in allowed_thread_ids:
            ...
            return f"[🐯Panel 𝑳𝒙𝒙𝒓𝒚𝒛 X Bot {bot.version}] đã được bật trong Group: {group.name} - ID: {thread_id}\n➜ Gõ lệnh ➡️ menuv1 hoặc bot để xem danh sách tính năng Vip💡"
    except Exception as e:
        print(f"Error: {e}")
        return "Có Lỗi Rồi Thằng Ngu ☠️"
...
def bot_off_group(bot, thread_id):
    ...
    try:
        ...
        if thread_id in allowed_thread_ids:
            ...
            return f"[🐯 Panel Bot {bot.version}] đã được tắt trong Group: {group.name} - ID: {thread_id}\n➜ Chào tạm biệt chúc bạn luôn may mắn🍀"
    except Exception as e:
        print(f"Error: {e}")
        return "Có Lỗi Rồi Thằng Ngu ☠️"
...
def remove_forbidden_word(word):
    """Xóa một từ khỏi danh sách từ ngữ cấm☃️."""
    settings = read_settings()
    forbidden_words = settings.get('forbidden_words', [])
    ...
    if word in forbidden_words:
        ...
        settings['forbidden_words'] = forbidden_words
        write_settings(settings)
        return f"➜ Từ '{word}' đã được xóa khỏi danh sách từ cấm ☃️"
    else:
    ...
    def send_check_profanity_response():
        ...
        for muted_user in muted_users:
            if muted_user['author_id'] == author_id and muted_user['thread_id'] == thread_id:
                if current_time >= muted_user['muted_until']:
                    ...
                    write_settings(settings)
                    response = "➜ 👾 Bạn đã được phép sủa! Hãy sủa thật 💬 lịch sự nhé! 🐯👍"
                    bot.replyMessage(Message(text=response), message_object, thread_id=thread_id, thread_type=thread_type)
                bot.deleteGroupMsg(message_object.msgId, author_id, message_object.cliMsgId, thread_id)
                return
        ...
        if any(word.lower() in message_text.lower() for word in forbidden_words):
            ...
            if penalty_level >= 2:
                ...
                response = f"➜ ⛔ Sv này đã bị cút khỏi nhóm do vi phạm nhiều lần\n➜ 💢 Nội dung vi phạm: sủa nhiều vcl : 🤬 '{message_text}'"
                bot.replyMessage(Message(text=response), message_object, thread_id=thread_id, thread_type=thread_type)
...
def print_muted_users_in_group(bot, thread_id):
    ...
    if muted_users_list:
        result = "➜ 🚫 Danh sách các noti bị khóa mõm: ☠️\n"
...
def add_users_to_ban_list(bot, author_ids, thread_id, reason):
    ...
    for author_id in author_ids:
        ...
        response += f"➜ 🚫 {user} đã bị cấm sủa trong {duration_minutes} ⏳ phút\n"
...
def remove_users_from_ban_list(bot, author_ids, thread_id):
    ...
    for author_id in author_ids:
        ...
        if (initial_count != len(muted_users)) or removed:
            response += f"➜ 🎉 Chúc mừng noti {user} đã được phép phát sủa 😂\n"
        else:
...
def unblock_users_from_group(bot, author_ids, thread_id):
    ...
    if unblocked_users:
        ...
        response = "➜ Không có noti nào bị chặn trong nhóm 😂"
...
def kick_users_from_group(bot, uids, thread_id):
    ...
    for uid in uids:
        try:
            ...
            response += f"➜ 💪 Đã kick con 🐶 😫 {user_name} khỏi nhóm thành công ✅\n"
        except Exception as e:
           
            user_name = get_user_name_by_id(bot, uid)
            response += f"➜ 😲 Mày kick đc à ?😫 {user_name} khỏi nhóm 😝\n"
...
def add_admin(bot, author_id, mentioned_uids, settings):
    ...
    for uid in mentioned_uids:
        if author_id not in admin_bot:
            response = "➜ 𝑲𝒉𝒐̂𝒏𝒈 𝑷𝒉𝒂̉𝒊 𝑳𝒙𝒙𝒓𝒚𝒛 𝑴𝒂̀ Đ𝒐̀𝒊 𝑺𝒂̀𝒊 𝑨̀⚡️"
...
def handle_bot_command(message, message_object, thread_id, thread_type, author_id, bot):
    def send_bot_response():
        try:
            ...
            if len(parts) == 1:
                response = (
                    "🌊𝑺𝒊𝒆̂𝒖 𝑷𝒉𝒂̂𝒎 𝑩𝒐𝒕 2024 𝒙 𝑳𝒙𝒙𝒓𝒚𝒛 ┇𝑪𝒉𝒂̀𝒐 𝒎𝒖̛̀𝒏𝒈 ⋆｡ﾟ☁︎｡⋆｡ ﾟ｡⋆ ! \n𝑫𝒖̛𝒐̛́𝒊 Đ𝒂̂𝒚 𝑳𝒂̀ 𝑪𝒂́𝒄 𝑪𝒉𝒖̛́𝒄 𝑵𝒂̆𝒏𝒈 𝑽𝒊𝒑 👇\n────୨ৎ────\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐢𝐧𝐟𝐨 :💻 𝐗𝐞𝐦 𝐭𝐡𝐨̂𝐧𝐠 𝐭𝐢𝐧 𝐜𝐡𝐢 𝐭𝐢𝐞̂́𝐭 𝐯𝐞̂̀ 𝐁𝐨𝐭 ☃️ᰔᩚ\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐨𝐧 𝐡𝐨𝐚̣̆𝐜 𝐨𝐟𝐟 : 🐽 𝐁𝐚̣̂𝐭/ 💨 𝐓𝐚̆́𝐭 𝐁𝐎𝐓 𝐭𝐫𝐨𝐧𝐠 𝐆𝐫𝐨𝐮𝐩 🥀\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐚𝐝𝐦𝐢𝐧 𝐚𝐝𝐝/𝐫𝐞𝐦𝐨𝐯𝐞/𝐥𝐢𝐬𝐭:✨ 𝐓𝐡𝐞̂𝐦/𝐱𝐨́𝐚/𝐱𝐞𝐦 𝐝𝐚𝐧𝐡 𝐬𝐚́𝐜𝐡 𝐀𝐝𝐦𝐢𝐧 ♱\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐧𝐨𝐢𝐪𝐮𝐲: ⛅ 𝐗𝐞𝐦 𝐧𝐨̣̂𝐢 𝐪𝐮𝐲 𝐆𝐫𝐨𝐮𝐩 ᥫᩣ\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯 𝐛𝐨𝐭 𝐛𝐚𝐧/𝐮𝐧𝐛𝐚𝐧 𝐥𝐢𝐬𝐭: 🚫 𝐃𝐚𝐧𝐡 𝐬𝐚́𝐜𝐡/ 🧸 𝐊𝐡𝐨́𝐚 / 💕 𝐌𝐨̛̉ 𝐦𝐨̃𝐦 𝐧𝐠𝐮̛𝐨̛̀𝐢 𝐝𝐮̀𝐧𝐠 ⋆𐙚₊˚⊹♡\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐤𝐢𝐜𝐤: 💤𝐂𝐡𝐨 𝟏 𝐬𝐯 𝐜𝐮́𝐭 𝐫𝐚 𝐤𝐡𝐨̉𝐢 𝐧𝐡𝐨́𝐦 °˚ ༘ ೀ⋆｡˚\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐛𝐥𝐨𝐜𝐤/𝐮𝐧𝐛𝐥𝐨𝐜𝐤/𝐥𝐢𝐬𝐭: ❄️ 𝐂𝐡𝐚̣̆𝐧 𝟏 𝐧𝐨𝐭𝐢 𝐤𝐡𝐨̉𝐢 𝐧𝐡𝐨́𝐦 ᯓᡣ𐭩\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐬𝐞𝐭𝐮𝐩 𝐨𝐧/𝐨𝐟𝐟: ⚙️ 𝐁𝐚̣̂𝐭/𝐓𝐚̆́𝐭 𝐧𝐨̣̂𝐢 𝐪𝐮𝐲 𝐧𝐨̣̂𝐢 𝐪𝐮𝐲 ⋅˚₊ ୨୧ ‧₊˚ ⋅\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐥𝐢𝐧𝐤 𝐨𝐧/𝐨𝐟𝐟: 🔗 𝐁𝐚̣̂𝐭/𝐓𝐚̆́𝐭 𝐜𝐡𝐨 𝐩𝐡𝐞́𝐩 𝐠𝐮̛̃𝐢 𝐥𝐢𝐧𝐤 𝐧𝐡𝐨́𝐦 ✮ ⋆ ˚｡𖦹 ⋆｡°✩\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐫𝐮𝐥𝐞 𝐰𝐨𝐫𝐝 : 📖 𝐐𝐮𝐲 đ𝐢̣𝐧𝐡 𝐜𝐚̂́𝐦 𝐧 𝐥𝐚̂̀𝐧 𝐯𝐢 𝐩𝐡𝐚̣𝐦, 𝐩𝐡𝐚̣𝐭 𝐦 𝐩𝐡𝐮́𝐭 🂡\n------------------------------------------------------------------------------\n"
                    "   ➜ 𝐋𝐞̣̂𝐧𝐡 🐯𝐛𝐨𝐭 𝐰𝐨𝐫𝐝 𝐚𝐝𝐝/𝐫𝐞𝐦𝐨𝐯𝐞 [𝐭𝐮̛̀ 𝐜𝐚̂́𝐦]: ☠️ 𝐓𝐡𝐞̂𝐦/𝐱𝐨́𝐚 𝐭𝐮̛̀ 𝐜𝐚̂́𝐦 ⋆˚✿˖°\n------------------------------------------------------------------------------\n\n"
                    "💤  𝒍𝒖𝒐̂𝒏 𝒔𝒂̆̃𝒏𝒈 𝒔𝒂̀𝒏 𝒑𝒉𝒖̣𝒄 𝒗𝒖̣ 𝒃𝒂̣𝒏𝒏 💞 🌸"
                )
                ...
                if action == 'on':
                    if not is_admin(author_id):
                        ...
                        response = "➜ Trong Group mới sài đc cu ⚡"
                    else:
                    ...
                    response = (
                        f"➜ 💻 Phiên bản: 1.0.2\n"
                        f"➜ 📅 Ngày cập nhật: 29/10/2024\n"
                        f"➜ 👨‍💻 Tác giả: 𝑳𝒙𝒙𝒓𝒚𝒛 🐯 💢 (Slam)\n"
                    ...
                    if len(parts) < 3:
                        ...
                        if sub_action == 'add':
                            if len(parts) < 4:
                                ...
                                if author_id not in admin_bot:
                                    response = "➜ Không phải 𝑳𝒙𝒙𝒓𝒚𝒛 Mà Đòi Sài Hả Con 🐶"
                                else:
                            ...
                            if admin_bot:
                                response = "[ DANH SÁCH ADMIN BOT 𝑳𝒙𝒙𝒓𝒚𝒛 X BOT 💻 ]\n\n"
                            ...
                            response = f"➜ Lệnh /bot admin {sub_action} không được hỗ trợ ☠️"
                    ...
                    if len(parts) < 3:
                        response = "➜ Vui lòng nhập [on/off] sau lệnh: /bot link ☃️\n➜ Ví dụ: /bot link on hoặc /bot link off ⛅"
                    else:
                    ...
                    if admins:
                        response = (
                            f"➜ 💢 Nội quy 💌Bot Vip Pro của 𝑳𝒙𝒙𝒓𝒚𝒛 được áp dụng cho nhóm: {group.name} - ID: {thread_id} ✅\n"
                            f"➜ 🚫 Cấm Sủa 🤬 trong nhóm\n"
                            f"➜ 💢 Vi phạm {threshold_word} lần sẽ bị bịt mõm {duration_word} phút\n"
                            f"➜ ⚠️ Nếu tái phạm 2 lần sẽ bị 💪 Cút khỏi nhóm 😂"
                        )
                    else:
                        response = (
                            f"➜ 💢 Nội quy không áp dụng cho nhóm: {group.name} - ID: {thread_id} 💔\n➜ Lý do: 🐯💢 𝑳𝒙𝒙𝒓𝒚𝒛 chưa được setup hoặc BOT không có quyền cầm key quản trị nhóm 💤"
                        )
                elif action == 'ban':
                    ...
                    if len(parts) < 3:
                        ...
                        if not is_admin(author_id):
                            ...
                            response = "➜ Trong Group mới sài đc cu ⚡"
                                ...
                                response = "➜ Lệnh này không khả thi do 🤖BOT không có quyền cầm 🔑 key nhóm ⛈️"
                        else:
                            uids = extract_uids_from_mentions(message_object)
